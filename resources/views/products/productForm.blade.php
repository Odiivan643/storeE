            ...
            <form
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" name="categories[]" id="categories" multiple>
                        <option disabled value="null"> Choisir une ou plusieurs catégory</option>
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" value="{{ $category->id }}"
                                @if (in_array($category->id, old('categories', isset($product) ? $product->categories->pluck('id')->toArray() : []))) selected @endif>{{ $category->name }}</option>
                        @endforeach
                    </select>
                    @error('categories')
                        <div class="error text-danger">
                ...
                <div class="mb-3 d-flex gap-2">
                    <label for="isIphone" class="form-label">IsIphone</label>
                    <div class="form-check form-switch">
                        <input name="isIphone" id="isIphone" value="true" data-bs-toggle="toggle"
                            {{ old('isIphone', isset($product) && $product->isIphone == 'true' ? 'checked' : '') }}
                            class="form-check-input" type="checkbox" role="switch" />
                    </div>
                    {{-- <select class="form-control" name="isIphone" id="isIphone">
                <option value="true" {{ old('isIphone', isset($product) && $product->isIphone == 'true' ? 'selected' : '') }}>Yes</option>
                <option value="false" {{ old('isIphone', isset($product) && $product->isIphone == 'false' ? 'selected' : '') }}>No</option>
            </select> --}}
                ...
                <div class="mb-3 d-flex gap-2">
                    <label for="isSamsung" class="form-label">IsSamsung</label>
                    <div class="form-check form-switch">
                        <input name="isSamsung" id="isSamsung" value="true" data-bs-toggle="toggle"
                            {{ old('isSamsung', isset($product) && $product->isSamsung == 'true' ? 'checked' : '') }}
                            class="form-check-input" type="checkbox" role="switch" />
                    </div>
                    {{-- <select class="form-control" name="isSamsung" id="isSamsung">
            <option value="true" {{ old('isSamsung', isset($product) && $product->isSamsung == 'true' ? 'selected' : '') }}>Yes</option>
            <option value="false" {{ old('isSamsung', isset($product) && $product->isSamsung == 'false' ? 'selected' : '') }}>No</option>
        </select> --}}
                ...
                <div class="mb-3 d-flex gap-2">
                    <label for="isHuawei" class="form-label">IsHuawei</label>
                    <div class="form-check form-switch">
                        <input name="isHuawei" id="isHuawei" value="true" data-bs-toggle="toggle"
                            {{ old('isHuawei', isset($product) && $product->isHuawei == 'true' ? 'checked' : '') }}
                            class="form-check-input" type="checkbox" role="switch" />
                    </div>
                    {{-- <select class="form-control" name="isHuawei" id="isHuawei">
            <option value="true" {{ old('isHuawei', isset($product) && $product->isHuawei == 'true' ? 'selected' : '') }}>Yes</option>
            <option value="false" {{ old('isHuawei', isset($product) && $product->isHuawei == 'false' ? 'selected' : '') }}>No</option>
        </select> --}}
