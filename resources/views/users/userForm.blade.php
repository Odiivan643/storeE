    ...
    <div class="row">
        <div class="col-md-8">
            <form
                ...
                <div class="mb-3">
                    {{-- Cacher le mot de pass de l'utilisateur au moment de l'edit  --}}
                    ...
                    @enderror --}}
                     {{-- Ajouter le champs select pour l'administrations de role  --}}
                    <div class="mb-3">
                        <label for="roles" class="form-label">role</label>
                        <select class="form-control" name="roles[]" id="roles" multiple>
                            <option disabled value="null"> Choisir une ou plusieurs catégory</option>
                            @foreach ($roles as $role)
                                <option value="{{ $role->id }}" value="{{ $role->id }}"
                                    @if (in_array($role->id, old('roles', isset($user) ? $user->roles->pluck('id')->toArray() : []))) selected @endif>{{ $role->name }}</option>
                            @endforeach
                        </select>
                        @error('roles')
                            <div class="error text-danger">
